...
For this project, I created a logic circuit and used VHDL code to implement the following circuit whose outputs is *LED1* and *LED0*. There are three switches available. I want LED1 **ON** when only one of the switches is in the **ON** position. I want LED0 **ON** only when the three switches are in the **ON** position.
